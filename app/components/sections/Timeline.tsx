...
export function Timeline() {
  ...
  useEffect(() => {
    if (isMobile) return; // vertical layout on mobile, no GSAP needed
  ...
  return (
    <section
      ...
      ref={sectionRef}
      className={isMobile ? "relative bg-bg py-16" : "relative h-screen overflow-hidden bg-bg"}
    >
      <div className={isMobile ? "px-5 pb-6 text-center" : "absolute inset-x-0 top-12 z-10 px-6 text-center"}>
      ...
      {isMobile ? (
        <div className="mt-10 flex flex-col gap-8 px-5">
          {TIMELINE.map((m) => (
            <article key={m.year} className="flex w-full flex-col gap-4">
              <div className="relative h-40 w-full overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-accent/30 via-surface to-black">
              ...
              <div>
                <p className="font-mono text-4xl font-light text-accent">{m.year}</p>
                <p className="mt-2 text-lg font-light">{m.title}</p>
                <p className="mt-1 text-sm text-text-muted">{m.sub}</p>
              </div>
            </article>
          ))}
        </div>
